...
class BulletProof
{
    ...
    protected function applyWatermark($imageName)
    {
        ...
        imagecopy($createImage, $watermark, imagesx($createImage) - $sx - $marginRight,
          imagesy($createImage) - $sy - $marginBottom, 0, 0, imagesx($watermark), imagesy($watermark));
    ...
    protected function applyShrink($fileName, $imageName)
    {
        ...
        imagecopyresampled($tmp, $image, 0, 0, 0, 0, $newImage["width"], $newImage["height"], $oldImage["width"],
          $oldImage["height"]);
    ...
    protected function applyCrop($imageName, $tmp_name)
    {
        ...
        imagecopyresampled($temp, $imageCreate, 0, 0, $widthTrim, $heightTrim, $crop["width"], $crop["height"],
    ...
    public function change($action, $imageName)
    {
        ...
        if ($action == "watermark" && $this->getWatermark) {
            $this->applyWatermark($imageName);

            return true;
        }

        if ($action == "shrink" && $this->shrinkImageTo) {
            $this->applyShrink($imageName, $imageName);

            return true;
        }

        if ($action == "crop" && $this->cropImageTo) {
    ...
    public function upload($fileToUpload, $isNameProvided = null)
    {
        ...
        if ($fileToUpload["size"] < $this->imageSize["min"] || $fileToUpload["size"] > $this->imageSize["max"]) {
            throw new ImageUploaderException("File sizes must be between " . implode(" to ",
        ...
        if ($pixel["height"] > $this->imageDimension["height"] || $pixel["width"] > $this->imageDimension["width"]) {
            throw new ImageUploaderException("Image pixels/size must be below " . implode(", ",
                $this->imageDimension) . " pixels");
        }
