...
if (isset($_POST['deleteFile'])) {
  ...
  $privParent = $oblak->checkUserPrivilegeLevelFiles(activeInstance, $oblak->getFileData(activeInstance, $fileID)['parent'], userID);
  if (($priv > 4)&&($privParent > 4)) {
    $oblak->throwError('forbidden');
  }
  if ($_POST['csrfToken'] == csrfToken) {
    $parent = $oblak->getFileData(activeInstance, $_POST['deleteFile'])['parentID'];
...
if (isset($_POST['updateFile'])) {
  if ($_POST['csrfToken'] == csrfToken) {
    ...
    $privParent = $oblak->checkUserPrivilegeLevelFiles(activeInstance, $_POST['parent'], userID);


    if (($priv > 4) && ($privParent > 4)) {
      $oblak->throwError('forbidden');
    }
    $oblak->updateFile(activeInstance, $_POST['updateFile'], $_FILES, userID, sessionID, $_POST['fileName'], $_POST['parent']);
...
if (stripos($routeSegments[1], "datoteke") !== false) {
  if (!empty($routeSegments[2])) {
    if($routeSegments[2] == 'sharing'){
      $pal = $oblak->addPublicShare(activeInstance, $routeSegments[3]);
      $view = 'ShareView';
    ...
    elseif ($routeSegments[2] == 'edit') {
      ...
      $folders = $oblak->createTreeOutput($oblak->getAllFoldersAUserCanAccess(activeInstance, userID));
      $fileData = $oblak->getFileData(activeInstance, $fileID);
      $view = 'UpdateView';
    }
    ...
    elseif ($routeSegments[2] == 'runtest') {
      //phpinfo();
      //var_dump($oblak->createTreeOutput($oblak->getAllFoldersAUserCanAccess(activeInstance, userID)));
    }
    else {
      $folderID = $routeSegments[3];
      $priv = $oblak->checkUserPrivilegeLevelFiles(activeInstance, $folderID, userID);
